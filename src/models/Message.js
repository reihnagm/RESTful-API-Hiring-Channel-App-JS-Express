...
module.exports = {
  ...
  conversationLists: (userLoggedIn) => {
    return new Promise ((resolve, reject) => {
      const query = `SELECT DISTINCT a.id, b.id, d.avatar, a.name 
      FROM user a, conversations b, conversation_replies c, engineer d 
      WHERE  
      CASE 
  ...
  conversationReplies: (id) => {
    return new Promise ((resolve, reject) => {
      ...
      WHERE a.user_id = b.id AND a.conversation_id = '${id}'`
      connection.query(query, (error, result) => {
        if(error) {
          reject(new Error(error))
        } else {
  ...
  storeConversations: (uid, user_one, user_two) => {
    return new Promise ((resolve, reject) => {
      const query = `INSERT INTO conversations (user_one, user_two)
      VALUES ('${uid}','${user_one}','${user_two}')`
      connection.query(query, (error, result) => {
        if(error) {
          reject(new Error(error))
        } else {
